...
export function ExameGrid({ exames, onDeleteClick }: Props) {
  ...
  // Se não houver exames, exibe uma mensagem
  if (exames.length === 0) {
    return <p className="text-muted-foreground">Nenhum exame encontrado.</p>;
  }

  // Navega para a página de edição do exame
      ...
      {exames.map((exame) => {
        ...
        return (
          ...
          >
            <CardContent className="flex p-0 overflow-hidden h-full">
              ...
              <div className="flex flex-col gap-1 py-5 pl-5 pr-4 flex-grow">
                ...
                <div
                    ...
                    onClick={(e) => e.stopPropagation()} // Impede que o clique nos botões propague para o card
                >
                    <Button
                      variant="outline"
                      size="sm"
            ...
            {/* A funcionalidade de pré-visualização foi removida */}
          </Card>
        );
      })}
    </div>
