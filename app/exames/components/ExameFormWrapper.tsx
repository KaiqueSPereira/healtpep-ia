...
export function ExameFormWrapper({
  ...
  useEffect(() => {
    if (existingExamData) {
      ...
      console.log(
        "exameResultados state after setting from existing data:",
        existingExamData.resultados,
      );
      setSelectedConsulta(existingExamData.consulta || null);
      console.log(
        "Inspecting existingExamData.consulta before setSelectedConsulta:",
        existingExamData.consulta,
      );
      setSelectedProfissional(existingExamData.profissional || null);
      console.log(
        "Value being used for setSelectedProfissional:",
        existingExamData.profissional,
      );
      setSelectedUnidade(existingExamData.unidades || null);
      console.log(
        "Value being used for setSelectedUnidade:",
        existingExamData.unidades,
      );
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Limpa apenas os resultados e anotação, mantém o tipo de exame selecionado
    setExameResultados([]);
    setAnotacao("");
  };
  ...
  const handleAnalyzeFile = async () => {
    if (!selectedFile) {
      toast({
        title: "Por favor, selecione um arquivo primeiro.",
        variant: "destructive",
        duration: 5000,
      });
      return;
    ...
    if (!tipo) {
      toast({
        title: "Por favor, selecione o tipo de exame primeiro.",
        variant: "destructive",
        duration: 5000,
      });
      return;
    ...
    formData.append("tipoExame", tipo); // Envia o tipo de exame para a API
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast({
          ...
          description: errorData.error || "Ocorreu um erro na análise.",
          variant: "destructive",
          duration: 5000,
        });
        setExameResultados([]);
      ...
      if (data.resultados?.length) {
        setExameResultados(
          data.resultados.map((res: ApiExameResult) => ({
            ...
            id: crypto.randomUUID(),
            outraUnidade: "",
            // Corrigido para mapear 'valorReferencia' da API para 'referencia' no estado
            referencia: res.valorReferencia || "", 
            nome: res.nome || "",
            valor: res.valor || "",
            unidade: res.unidade || "",
          })),
        ...
        toast({
          title: "Análise concluída com sucesso! Resultados extraídos.",
          variant: "default",
          duration: 5000,
        });
      } else {
        toast({
          title:
            "Análise concluída. Nenhum resultado estruturado foi encontrado.",
          variant: "default",
          duration: 5000,
        });
        setExameResultados([]);
      ...
      toast({
        title: "Ocorreu um erro inesperado durante a análise.",
        variant: "destructive",
        duration: 5000,
      });
      setExameResultados([]);
  ...
  const handleSubmit = async () => {
    ...
    const analysisDoneAndResultsFound = needsAnalysis
      ? exameResultados?.length > 0
      : true;
    ...
    if (
      ...
      if (!userId)
        errorMessage = "Erro de autenticação. Por favor, recarregue a página.";
      else if (!selectedProfissional?.id)
        errorMessage = "Por favor, selecione um profissional.";
      else if (!selectedUnidade?.id)
        errorMessage = "Por favor, selecione uma unidade.";
      else if (!existingExamData && !selectedFile)
      ...
      else if (!selectedConsulta?.id && !selectedTratamento?.id)
        errorMessage = "Por favor, selecione uma consulta ou um tratamento.";
      else if (
        needsAnalysis &&
        !analysisDoneAndResultsFound &&
        !existingExamData
      )
        errorMessage =
          "Por favor, analise o arquivo e garanta que resultados foram extraídos para este tipo de exame.";
    ...
    try {
      ...
      else formData.append("consultaId", "");
      if (selectedTratamento?.id)
        formData.append("tratamentoId", selectedTratamento.id);
      else formData.append("tratamentoId", "");
      ...
      if (res.ok) {
        ...
        toast({
          title: existingExamData
            ...
            : "Erro ao enviar o exame",
          description:
            errorData.error ||
            (existingExamData
              ? "Ocorreu um erro ao atualizar o exame."
              : "Ocorreu um erro ao salvar o exame."),
          variant: "destructive",
          duration: 5000,
        });
      ...
      <main className="container mx-auto space-y-8 py-8">
        ...
        >
          ...
          <ExamDetailsForm
            ...
            onTipoChange={
              (value) => {
                // Add console log and update state
          ...
          >
            <Button
              disabled={
                ...
                (["Sangue", "Urina"].includes(tipo) &&
                  !existingExamData &&
                  (!exameResultados || exameResultados.length === 0)) // Validate analysis and results only for new exams of types that need analysis
              }
              className="w-full"
            >
              {loadingSubmit
