...
import { ChevronLeftIcon, Edit } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/app/_components/ui/tabs";
import { Consulta, TimelineItem, Anotacao, Anexo } from '@/app/_components/types';
...
const ConsultaPage = () => {
    ...
    const timelineUnificada: TimelineItem[] = useMemo(() => {
        ...
        const consultas: TimelineItem[] = historico
            ...
            .map((item: HistoricoTratamentoItem) => ({ id: item.id, data: item.data, tipo: item.tipo, motivo: item.motivo || 'Consulta de rotina', profissional: item.profissional, unidade: item.unidade, entryType: 'consulta', href: `/consulta/${item.id}`, anotacao: null }));
        ...
        const exames: TimelineItem[] = examesDoContexto
            ...
            .map((exame: ExameComRelacoes) => ({ id: exame.id, data: exame.dataExame!, tipo: exame.tipo || 'Exame', motivo: 'Solicitação de Exame', profissional: exame.profissional, unidade: exame.unidades, entryType: 'exame', href: `/exames/${exame.id}`, anotacao: exame.anotacao }));
    ...
    const handleAdicionarAnotacao = async () => {
        ...
        try {
            ...
            setConsulta(prev => prev ? { ...prev, Anotacoes: [...(prev.Anotacoes || []), novaAnotacao] } : null);
    ...
    const handleUpdateAnotacao = async (anotacaoId: string, newContent: string) => {
        ...
        try {
            ...
            setConsulta(prev => {
                if (!prev) return null;
                const updatedAnotacoes = prev.Anotacoes?.map((a: Anotacao) => a.id === anotacaoId ? updatedAnotacao : a);
                return { ...prev, Anotacoes: updatedAnotacoes };
            });
    ...
    const handleDeleteAnotacao = async (anotacaoId: string) => {
        if (!consulta) return;
        try {
            const response = await fetch(`/api/consultas/${consulta.id}/anotacoes/${anotacaoId}`, { method: 'DELETE' });
                    ...
                    <div className="w-full lg:w-2/3 space-y-6">
                        <Tabs defaultValue="geral" className="w-full">
                            ...
                            <TabsContent value="geral" className="space-y-6 pt-4">
                                ...
                                <AnexosCard consultaId={consulta.id} initialAnexos={consulta.anexos} />
                            </TabsContent>
